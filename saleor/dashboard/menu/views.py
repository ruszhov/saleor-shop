...
from .forms import AssignMenuForm, MenuForm, MenuItemForm, ReorderMenuItemsForm, MenuItemTranslationForm
...
@staff_member_required
@permission_required('menuitem.manage_products')
def menuitem_translation_create(request, menu_pk):
    menuitem = get_object_or_404(MenuItem.objects.all(), pk=menu_pk)
    form = MenuItemTranslationForm(request.POST or None)
    if form.is_valid():
        cat_trans = form.save(commit=False)
        cat_trans.id = MenuItemTranslation.objects.all().aggregate(Max('id'))['id__max'] + 1
        cat_trans.menu_item_id = menu_pk
        cat_trans.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved translation %s') % (menuitem.name,)
        messages.success(request, msg)
        return redirect(
            'dashboard:menu-item-details', menu_pk=menuitem.menu_id, item_pk=menuitem.pk )
    ctx = {'form': form, 'menuitem': menuitem}
    return TemplateResponse(
        request,
        'dashboard/menu/item/menuitem_translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('menuitem.manage_products')
def menuitem_translation_edit(request, menu_pk, translation_pk):
    menuitem = get_object_or_404(MenuItem.objects.all(), pk=menu_pk)
    translation = get_object_or_404(menuitem.translations.all(), pk=translation_pk)
    form = MenuItemTranslationForm(request.POST or None, instance=translation)
    if form.is_valid():
        form.save()
        msg = pgettext_lazy(
        ...
        messages.success(request, msg)
        return redirect(
            'dashboard:menu-item-details', menu_pk=menuitem.menu_id,
            item_pk=menuitem.pk)
...
def menuitem_translation_delete(request, menu_pk, translation_pk):
    menuitem = get_object_or_404(MenuItem, pk=menu_pk)
    translation = get_object_or_404(menuitem.translations, pk=translation_pk)
    if request.method == 'POST':
        translation.delete()
        msg = pgettext_lazy(
        ...
        messages.success(request, msg)
        return redirect(
            'dashboard:menu-item-details', menu_pk=menuitem.menu.pk,
            item_pk=menuitem.pk)
    ctx = {
        'menuitem': menuitem,
    ...
    return TemplateResponse(
        request,
        'dashboard/menu/item/menuitem_translation/modal/confirm_delete.html',
        ctx)
...
def menu_item_details(request, menu_pk, item_pk):
    menu = get_object_or_404(Menu, pk=menu_pk)
    menu_item = get_object_or_404(menu.items.all(), pk=item_pk)
    translations = MenuItemTranslation.objects.filter(menu_item_id = item_pk)
    path = menu_item.get_ancestors(include_self=True)
    ...
    menu_items = get_paginator_items(
        ...
        request.GET.get('page'))
    ctx = {
        'menu': menu, 'menu_item': menu_item, 'menu_items': menu_items, 'translations': translations,
...
@staff_member_required
@permission_required('product.manage_products')
def menuitem_translation_details(request, menu_pk, translation_pk):
    menuitem = get_object_or_404(MenuItem, pk=menu_pk)
    translation = get_object_or_404(menuitem.translations.all(), pk=translation_pk)

    ctx = {
        'menuitem': menuitem, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/menu/item/menuitem_translation/detail.html',
        ctx)
