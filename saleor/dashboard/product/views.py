...
from . import forms
from . forms import AttributeTranslationForm, AttributeValueTranslationForm
...
def product_details(request, pk):
    products = Product.objects.prefetch_related('variants', 'images', 'translations').all()
    ...
    ctx = {
        ...
        'discounted_price': discounted_price, 'variants': variants, 'translations':translations,
...
def variant_details(request, product_pk, variant_pk):
    ...
    translation = variant.translations.all()
    ...
    ctx = {
        'images': images, 'product': product, 'variant': variant, 'translations':translation,
...
def variant_translation_details(request, product_pk, variant_pk, translation_pk):
    variant = get_object_or_404(ProductVariant, pk=variant_pk)
    translation = get_object_or_404(variant.translations.all(), pk=translation_pk)

    # If the product type of this product assumes no variants, redirect to
    # product details page that has special UI for products without variants.

    ctx = {
        'variant': variant, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/product_variant/translation/detail.html',
        ctx)
...
@staff_member_required
@permission_required('product.manage_products')
def variant_translation_edit(request, variant_pk, translation_pk, product_pk):
    variant = get_object_or_404(ProductVariant.objects.all(), pk=variant_pk)
    translation = get_object_or_404(variant.translations.all(), pk=translation_pk)
    form = forms.VariantTranslationForm(request.POST or None, instance=translation)
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect(
            'dashboard:variant-translation-details', variant_pk=variant.pk,
            translation_pk=translation.pk, product_pk=variant.product_id)
    ctx = {'form': form, 'variant': variant, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/product_variant/translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('product.manage_products')
def variant_translation_delete(request, variant_pk, translation_pk, product_pk):
    variant = get_object_or_404(ProductVariant, pk=variant_pk)
    translation = get_object_or_404(variant.translations, pk=translation_pk)
    if request.method == 'POST':
        translation.delete()
        msg = pgettext_lazy(
            'Dashboard message', 'Removed translation %s') % (translation.name,)
        messages.success(request, msg)
        return redirect('dashboard:variant-details', product_pk=product_pk, variant_pk=variant_pk)
    ctx = {
        'variant': variant,
        'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/product_variant/translation/modal/confirm_delete.html',
        ctx)
...
@staff_member_required
@permission_required('product.manage_products')
def translation_details(request, product_pk, translation_pk):
    product = get_object_or_404(Product, pk=product_pk)
    translation = get_object_or_404(product.translations.all(), pk=translation_pk)
...
@staff_member_required
@permission_required('product.manage_products')
def translation_edit(request, product_pk, translation_pk):
    product = get_object_or_404(Product.objects.all(), pk=product_pk)
    translation = get_object_or_404(product.translations.all(), pk=translation_pk)
    form = forms.ProductTranslationForm(request.POST or None, instance=translation)
    if form.is_valid():
        form.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved variant %s') % (translation.name,)
...
@staff_member_required
@permission_required('product.manage_products')
def translation_delete(request, product_pk, translation_pk):
    product = get_object_or_404(Product, pk=product_pk)
    translation = get_object_or_404(product.translations, pk=translation_pk)
    if request.method == 'POST':
        translation.delete()
        msg = pgettext_lazy(
            'Dashboard message', 'Removed translation %s') % (translation.name,)
    ...
    return TemplateResponse(
        request,
        'dashboard/product/product_translation/modal/confirm_delete.html',
        ctx)
...
def attribute_details(request, pk):
    attributes = Attribute.objects.prefetch_related(
        'values', 'product_type', 'product_variant_type', 'translations').all()
    attribute = get_object_or_404(attributes, pk=pk)
    translations = attribute.translations.all()
...
@staff_member_required
@permission_required('product.manage_products')
def attribute_value_details(request, attribute_pk, value_pk):
    attributes = Attribute.objects.prefetch_related(
        'values', 'product_type', 'product_variant_type', 'translations').all()
    attribute = get_object_or_404(Attribute, pk=attribute_pk)
    value = get_object_or_404(AttributeValue, pk=value_pk)
    translations = AttributeValueTranslation.objects.filter(attribute_value_id = value_pk)
    product_type = attribute.product_type or attribute.product_variant_type
    ctx = {
        'attribute': attribute, 'product_type': product_type, 'value': value,
        'translations': translations}
    return TemplateResponse(
        request, 'dashboard/product/attribute/values/detail.html', ctx)


@staff_member_required
@permission_required('product.manage_products')
def attribute_translation_details(request, attribute_pk, translation_pk):
    attribute = get_object_or_404(Attribute, pk=attribute_pk)
    translation = get_object_or_404(attribute.translations, pk=translation_pk)

    ctx = {
        'attribute': attribute, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/attribute/attribute_translation/detail.html',
        ctx)
...
def attribute_translation_create(request, attribute_pk):
    attribute = get_object_or_404(Attribute.objects.all(), pk=attribute_pk)
    form = AttributeTranslationForm(request.POST or None)
    if form.is_valid():
        cat_trans = form.save(commit=False)
        cat_trans.id = AttributeTranslation.objects.all().aggregate(Max('id'))['id__max'] + 1
        cat_trans.attribute_id = attribute_pk
        cat_trans.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved attribute %s') % (attribute.name,)
        messages.success(request, msg)
        return redirect(
            'dashboard:attribute-details', pk=attribute.pk)
    ctx = {'form': form, 'attribute': attribute}
    return TemplateResponse(
        request,
        'dashboard/product/attribute/attribute_translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('category.manage_products')
def attribute_translation_edit(request, attribute_pk, translation_pk):
    attribute = get_object_or_404(Attribute.objects.all(), pk=attribute_pk)
    translation = get_object_or_404(attribute.translations.all(), pk=translation_pk)
    form = AttributeTranslationForm(request.POST or None, instance=translation)
    if form.is_valid():
        form.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved variant %s') % (translation.name,)
        ...
        return redirect(
            'dashboard:attribute-details', pk=attribute.pk)
    ctx = {'form': form, 'attribute': attribute, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/attribute/attribute_translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('product.manage_products')
def attribute_translation_delete(request, attribute_pk, translation_pk):
    attribute = get_object_or_404(Attribute, pk=attribute_pk)
    translation = get_object_or_404(attribute.translations, pk=translation_pk)
    if request.method == 'POST':
        translation.delete()
    ...
    return TemplateResponse(
        request,
        'dashboard/product/attribute/attribute_translation/modal/confirm_delete.html',
        ctx)
...
@staff_member_required
@permission_required('category.manage_products')
def attributevalue_translation_create(request, attvalue_pk):
    value = get_object_or_404(AttributeValue.objects.all(), pk=attvalue_pk)
    form = AttributeValueTranslationForm(request.POST or None)
    if form.is_valid():
        cat_trans = form.save(commit=False)
        # cat_trans.id = AttributeValueTranslation.objects.all().aggregate(Max('id'))['id__max'] + 1
        cat_trans.attribute_value_id = attvalue_pk
        cat_trans.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved attribute %s') % (value.name,)
        messages.success(request, msg)
        return redirect(
            'dashboard:attribute-value-details', attribute_pk=value.attribute.pk, value_pk=value.pk)
    ctx = {'form': form, 'value': value}
    return TemplateResponse(
        request,
        'dashboard/product/attribute/values/translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('category.manage_products')
def attributevalue_translation_edit(request, attvalue_pk, translation_pk):
    value = get_object_or_404(AttributeValue.objects.all(), pk=attvalue_pk)
    translation = get_object_or_404(value.translations.all(), pk=translation_pk)
    form = AttributeValueTranslationForm(request.POST or None, instance=translation)
    if form.is_valid():
        form.save()
        msg = pgettext_lazy(
            'Dashboard message', 'Saved variant Value %s') % (translation.name,)
        messages.success(request, msg)
        return redirect(
            'dashboard:attribute-value-details', attribute_pk=value.attribute.pk, value_pk=value.pk)
    ctx = {'form': form, 'value': value, 'translation': translation}
    return TemplateResponse(
        request,
        'dashboard/product/attribute/values/translation/form.html',
        ctx)
...
@staff_member_required
@permission_required('product.manage_products')
def attributevalue_translation_delete(request, attvalue_pk, translation_pk):
    value = get_object_or_404(AttributeValue, pk=attvalue_pk)
    translation = get_object_or_404(value.translations, pk=translation_pk)
    if request.method == 'POST':
        translation.delete()
        msg = pgettext_lazy(
            'Dashboard message', 'Removed translation %s') % (translation.name,)
    ...
    return TemplateResponse(
        request,
        'dashboard/product/attribute/values/translation/confirm_delete.html',
        ctx)
